...
/** chatWithAnalysis 에 프론트엔드가 전달하는 분석 컨텍스트 */
...
/** generateWeeklyAnalysis 에 프론트엔드가 전달하는 일간 요약 */
...
function buildChatSystemPrompt(ctx: AnalysisContext): string {
...
export const analyzeScreenTime = onCall(
  ...
  async (request) => {
    ...
    // 분석 결과를 프론트엔드로 반환 — 저장은 프론트엔드가 /api/analyses 통해 처리
    return { analysisData };
  }
);
...
export const generateWeeklyAnalysis = onCall(
  ...
  async (request) => {
    ...
    const { dailySummaries } = request.data as { dailySummaries: unknown };

    // 입력 검증
    if (!Array.isArray(dailySummaries) || dailySummaries.length < 7) {
      throw new HttpsError(
        "failed-precondition",
        `이번 주(월~일) 일간 분석이 ${Array.isArray(dailySummaries) ? dailySummaries.length : 0}개 있습니다. 7개가 모여야 주간 분석을 시작할 수 있습니다.`
      );
    }
    if (dailySummaries.length > 7) {
      throw new HttpsError("invalid-argument", "dailySummaries는 최대 7개까지 허용됩니다.");
    }
    for (const s of dailySummaries) {
      if (
        ...
        throw new HttpsError("invalid-argument", "dailySummaries 형식이 올바르지 않습니다.");
      }
    }

    const weeklyPrompt = buildWeeklyPrompt(dailySummaries as DailySummary[]);
    ...
    // 분석 결과를 프론트엔드로 반환 — 저장은 프론트엔드가 /api/analyses 통해 처리
    return { analysisData: weeklyData };
  }
);
...
export const chatWithAnalysis = onCall(
  ...
  async (request) => {
    ...
    const uid = request.auth.uid;
    const { analysisId, messages, analysisContext } = request.data as {
      analysisId: string;
    ...
    if (
      ...
      (analysisContext.periodType !== "daily" && analysisContext.periodType !== "weekly") ||
      typeof analysisContext.totalMinutes !== "number" ||
      typeof analysisContext.detoxScore !== "number" ||
      !Array.isArray(analysisContext.apps) ||
      !Array.isArray(analysisContext.coreProblems)
    ) {
      throw new HttpsError("invalid-argument", "analysisContext 형식이 올바르지 않습니다.");
    }
